...
namespace MethodTracker.MockProject
{
    ...
    internal class ValidationService : IValidationService { public void Validate(OrderRequest req) { new RuleService().Run(req); } }
    internal class RuleService { public void Run(OrderRequest req) { new RuleStep1().Execute(req); } }
    internal class RuleStep1 { public void Execute(OrderRequest req) { new RuleStep2().Execute(req); } }
    ...
    internal class DiscountService : IDiscountService { public decimal Apply(string promo, decimal amount) { return promo == "VIP" ? amount * 0.9m : 0m; } }
    internal class TaxService : ITaxService { public decimal Apply(decimal amount) { return amount * 0.07m; } }
    internal class InventoryService : IInventoryService { public async Task<bool> ReserveAsync(List<int> products) { for (int i = 0; i < 3; i++) { await Task.Delay(10); new InventoryValidator().Check(products); } return true; } }
    internal class InventoryValidator { public void Check(List<int> products) { } }
    ...
    internal class ShippingEstimator { public void Estimate(ref decimal fee) { fee += 2.0m; } }
    ...
    internal class AnalyticsService : IAnalyticsService  { public void TrackOrder(Order o, PaymentResult p, bool s, List<string> r) { new AnalyticsDetail().Log(o, p, s, r); }
}
internal class AnalyticsDetail { public void Log(Order o, PaymentResult p, bool s, List<string> r) { int count = r.Count; } }
internal class AuditService : IAuditService { public void Record(Order o, User u, PaymentResult pr, bool s) { new AuditDetailService().Log(o, u, pr, s); } }
...
internal class EmailService { public void Send(User u, string oid) { new EmailFormatter().Format(oid); } }
internal class EmailFormatter { public void Format(string oid) { } }
internal class PushService { public void Send(User u, string oid) { new PushQueue().Enqueue(u.Id, oid); } }
